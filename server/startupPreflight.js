...
export function evaluateStartupPreflight(input) {
  ...
  if (!loopback) {
    errors.push(
      `Refusing non-loopback bind ${input.bindHost}: direct LAN access is not authenticated. Keep BIND_HOST=127.0.0.1 and use an SSH tunnel, authenticated TLS reverse proxy, or Tailscale Serve.`
    );
  }
  ...
  return {
    ...
    authMode: loopback ? "loopback-only" : "unsupported-direct",
    errors,
    warnings,
  };
}
