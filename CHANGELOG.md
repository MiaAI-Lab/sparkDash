...
### Fixed
- **SGLang live rates read the wrong series — the whole LLM card showed 0 tok/s while streaming, then one large spike** — decode/prefill tok/s were derived from `sglang:generation_tokens_total` / `sglang:prompt_tokens_total`, which SGLang increments only in `observe_one_finished_request()`, i.e. once per *finished* request. The card therefore read 0 for the entire stream and jumped to a single large number when the request landed. All four rates now come from `sglang:realtime_tokens_total`, which the scheduler republishes every log interval: `mode="decode"` → Generation tok/s, `prefill_compute` → Uncached prefill tok/s, `prefill_cache` → Cached prefill tok/s, and the two prefill modes sum to Prefill tok/s. The finish-only counters stay the source for the lifetime output total, and remain the fallback for older builds without the realtime series. The uncached prefill rate is also corrected on that fallback path: `prompt_tokens_total` is the *whole* prompt, so the computed part is now the remainder after the cached tokens (previously the two rates double-counted and `Cached + Uncached` exceeded `Prefill`). The Showcase/bench streaming sampler (`readServerGenerationTokens`) prefers the same live decode series.
