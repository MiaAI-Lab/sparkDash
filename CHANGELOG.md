...
### Added
- **Spark AutoPower — idle shutdown + scheduled wake** — new full-width card under the Model Launcher powers the whole Spark fleet off when it is genuinely idle and brings it back on a schedule, so the boxes stop burning watts overnight. Idle is judged only by the two things that actually use the Sparks: the AI proxy (0 in-flight streams and 0 in-flight requests across all clients) and the Spark Dev Engine (0 used slots, 0 active tickets, 0 queued/active plans) — each polled live every 30 s; any single non-zero counter, a plan still queued, or an unreachable source counts as busy, so a flaky bridge never powers the fleet off mid-job. After the counters stay quiet for the configured span (default 30 min) *inside* the watch window, the worker shuts down first and the head (which hosts the proxy) last; a scheduled WoL magic packet (head first, worker after) wakes them at the configured time. One watch span + one wake time per day type — weekdays and weekends, evaluated on the operator's clock (`Europe/Prague`); the window may wrap midnight (22:00 → 07:00 = the night). Wake fires with a 15-minute grace after its minute (a dashboard restart still wakes the box); an already-on Spark is never woken again. A large master toggle turns the whole automation on/off; the settings form edits idle span + both windows + both wake times with HH:MM validation (empty wake = no auto-wake). Server-restart safe: the idle timer and last-fired wake persist in `config/autopower-state.json`, and the dashboard host itself — which runs the dashboard — is never a target. A "tick now" button re-probes and reports the exact decision. Routes: `GET /api/autopower`, `PUT /api/autopower/config`, `POST /api/autopower/tick`.
...
- **Dashboard measurement cost cut ~28×** — with the dashboard open, sparkDash itself spiked the monitored machine by ~20 percentage points of CPU (measured: dashboard container at 143% of one core on an idle box), because every metric domain issued its own `sshExec` — a fresh TCP/KEX/auth handshake per command, ~6 per Spark per 2 s cycle. Remote SSH now multiplexes over one persisted OpenSSH `ControlMaster` per target (`ControlPersist=120`, keepalive-detected dead remotes, transparent stale-socket fallback), the five fast domains (gpu/cpu/ram/network/unified memory) are bundled into ONE SSH exec per Spark per cycle whose sections feed the unchanged per-domain parsers, the network link speed no longer costs a second round-trip, and the local path caches `ip -4 addr show` and the default-route interface for 30 s to skip per-tick `nsenter` forks. Post-fix with the dashboard open: 5% of one core, sparkDash CPU on the monitored host back to noise (22% → 2% displayed). Manual single-domain refresh on a remote Spark now refreshes the whole bundle (same exec, fresher).
- **CPU power is MEASURED on intel RAPL hosts** — the Power tile always showed a linear TDP estimate (`0.08·TDP + usage·0.92·TDP`), which floors at ~14.8 W and denominates by the generic 185 W fallback: an idle i3-10100 (true package draw ≈ 1 W) read "20.1W / 185W" and looked like a power hog. The privileged container reads `intel-rapl:0` (`energy_uj` deltas across two consecutive polls with `max_energy_range_uj` wrap correction, ≥0.5 s window; `constraint_0_power_limit_uw` — or the older `_uj` spelling rescaled — as the PL1 denominator), so the dell card now shows real package watts over the platform limit (e.g. "0.8W / 65W"). Remote Sparks get the same treatment: the CPU bundle section also cats their powercap triple — GB10 has no intel-RAPL, so the DGX Sparks keep the estimate path untouched (`source` field marks which is which). First poll after startup and hosts without readable counters degrade silently to the estimate.

### Fixed
- **Invisible dashboard outage from the watch-mode container** — the container ran `node --watch server/index.js` as its main process. When the watched child died (seen in the wild after a host reboot), the watch parent stayed alive, so `docker compose ps` reported *running* while nothing answered on 5555 — every route connection-refused, with no `die` event, no exit code, no restart. The compose command is now plain `node server/index.js`: a crashed server exits the container, `restart: always` relaunches it fresh, and a crash loop is visible in `docker compose ps` / `RestartCount` instead of masquerading as healthy; live-code edits from the bind mount apply on `docker compose restart`. Verified in production: kill the node process → container restarts → HTTP 200 again, `RestartCount` incremented.
- **vLLM statistics keep updating while HW graphs are paused** — the viewport-off-screen polling pause cleared every domain timer, including the LLM probe, so vLLM tokens/requests froze on cards whose spark's HW graphs were scrolled out of view. The pause now clears only the HW/system timers; the LLM probe (a cheap local HTTP fetch) keeps polling at its normal cadence in every state, and `resume()` re-arms it without duplication.
